...
public class CustomDSPerLevel {
    ...
    public Map<Double, List<Integer>> map_sorted_quartets_weight_list_indices;
    ...
    public CustomDSPerLevel(InitialTable initialTable) {
        ...
        this.map_sorted_quartets_weight_list_indices = new TreeMap<>(Collections.reverseOrder());
    ...
    private void printMap_RelevantQuartetsPeraTaxa() {
        ...
        for (String key_taxa : map_taxa_relevant_quartet_indices.keySet()) {
            ...
            System.out.print("Taxa:<" + key_taxa + ">: Length: " + list_relevant_qrts_indices.size() + "  ==>> ");
            for (int i = 0; i < list_relevant_qrts_indices.size(); i++) {
                System.out.print(this.table1_initial_table_of_quartets.getQuartetAt(list_relevant_qrts_indices.get(i)) + ",");
            }
            System.out.println("");
        }
    ...
    public void printCustomDS() {
        ...
        System.out.println(this.map_sorted_quartets_weight_list_indices);
    }
    ...
    public void sortQuartetIndicesMap() {
        for (int i = 0; i < this.quartet_indices_list_unsorted.size(); i++) {
            ...
            Quartet q = this.table1_initial_table_of_quartets.getQuartetAt(qrt_index);
            if (this.map_sorted_quartets_weight_list_indices.containsKey(q.weight) == false) { //initialize the list [this weight doesn't exist]
                this.map_sorted_quartets_weight_list_indices.put(q.weight, new ArrayList<>());
            }
            this.map_sorted_quartets_weight_list_indices.get(q.weight).add(qrt_index); //append to list in treeMap
        }
    }
    ...
    public void fillRelevantQuartetsMap() {
        ...
        for (int itr_quartet = 0; itr_quartet < this.quartet_indices_list_unsorted.size(); itr_quartet++) {
            ...
            Quartet q = this.table1_initial_table_of_quartets.getQuartetAt(index_qrt);
